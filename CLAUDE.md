...
Sieben der acht Regeln sind x14-Erweiterungsregeln; der Zugriff auf eine einzelne Regel
hat Excel hart abstürzen lassen. Das reine Zählen hat der Nutzer am 13.09.2026
freigegeben, weil die Warnung aus Issue #31 es braucht. Alles andere — Schreiben,
Enumerieren, `Formula1` lesen — bleibt verboten; `vbacheck.py` lässt nichts als `.Count`
durch.

PR #59 hat testweise zusätzlich `AppliesTo.Areas.Count` einer einzelnen Regel gelesen, um
einen Flächen-Zerfall der Regel zu erkennen. Zwei Ergebnisse, beide wichtig für einen
späteren Anlauf: **das Lesen crasht nicht** (bestätigt am Nutzer-Excel, 13.09.2026) — aber
der Flächen-Zerfall selbst ist **kein Fehler**, sondern das normale Nebenprodukt des in
diesem Projekt verwendeten Zeilen-Einfügens per „kopierte Zellen" (siehe
`Doku/Fallstricke.md`) und tritt auf jeder echten Mappe mit Ferienzeilen auf. Deshalb
wieder auf reines `.Count` zurückgebaut.
